...
from ensembl.production.metadata.admin.models import Dataset, DatasetType, DatasetSource, GenomeDataset,DatasetAttribute
...
# #POST:
    def create(self, request, *args, **kwargs):
        ...
        if existing_datasets.exists():
            return Response({
                'error': 'Dataset with the provided genome_uuid, dataset_type.name and dataset_source.name already exists and is not attached to a release. Please use PUT to update instead of POST.'
            }, status=status.HTTP_400_BAD_REQUEST)
